<template>
	<view class="login">
		<view class="content">
			...
			<!-- 主体表单 -->
			<view class="main">
				<wInput v-model="phone" type="text" maxlength="11" placeholder="用户名/电话" @input="phoneInput"></wInput>
				<wInput v-model="password" type="password" maxlength="11" placeholder="密码" isShowPass @input="passwordInpur"></wInput>
			</view>
			<wButton text="登 录" @tap="bindLogin"></wButton>

			<!-- 其他登录 -->
			<view class="oauth-row" v-if="hasProvider" v-bind:style="{ top: positionTop + 'px' }">
				<view class="oauth-image" v-for="provider in providerList" :key="provider.value">
					<image :src="provider.image" @tap="login_weixin()"></image>
					<!-- #ifdef MP-WEIXIN -->
			...
			<!-- 底部信息 -->
			<view class="footer">
				<navigator url="../pwd/pwd" open-type="navigate">找回密码</navigator>
				<text>|</text>
				<navigator url="../reg/reg" open-type="navigate">注册账号</navigator>
			</view>
		</view>
	</view>
</template>
			    ...
			    success() {
			        uni.getLocation({
			            ...
			            success: function (res) {
							...
							uni.request({
								url: 'http://127.0.0.1:5000/updatePosition',
								method: 'POST',
								data: {
									"stu_account":_that.userName,
